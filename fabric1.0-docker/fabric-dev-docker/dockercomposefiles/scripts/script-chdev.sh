...
instantiateChaincode () {
	...
	setGlobals $PEER
        if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		echo "peer chaincode instantiate -o fabric-dev-orderer:7050 -C $CHANNEL_NAME -n $CHAINCODE_ID -v $CHAINCODE_VER -c $CCINIT_ARGS -P \"OR('DEFAULT.member')\" >&log.txt "
		peer chaincode instantiate -o fabric-dev-orderer:7050 -C $CHANNEL_NAME -n $CHAINCODE_ID -v $CHAINCODE_VER -c $CCINIT_ARGS -P "OR('DEFAULT.member')" >&log.txt 
        else
		echo "peer chaincode instantiate -o fabric-dev-orderer:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CHAINCODE_ID -v $CHAINCODE_VER -c $CCINIT_ARGS -P \"OR('DEFAULT.member')\" >&log.txt"
		peer chaincode instantiate -o fabric-dev-orderer:7050 --tls $CORE_PEER_TLS_ENABLED --cafile $ORDERER_CA -C $CHANNEL_NAME -n $CHAINCODE_ID -v $CHAINCODE_VER -c $CCINIT_ARGS -P "OR('DEFAULT.member')" >&log.txt
	fi
	res=$?
	cat log.txt
